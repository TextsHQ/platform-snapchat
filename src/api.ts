...
import type { SnapchatUser } from './snapchat-types'
import type SnapchatPlatformInfo from './info'

const { Sentry } = texts

export default class Snapchat implements PlatformAPI {
  private readonly api = new SnapchatAPI()

  // private readonly live = new LivePipeline(this.api, this.onLiveEvent.bind(this))

  currentUser: SnapchatUser = null
  friendInfo: any = {}
  ...
  init = async (cookieJarJSON: any, _: AccountInfo, prefs: Record<keyof typeof SnapchatPlatformInfo['prefs'], any>) => {
  ...
  getThreads = async (folderName: ThreadFolderName, pagination: PaginationArg): Promise<Paginated<Thread>> => {
    let threads = await this.api.getThreads(this.currentUser.id, this.friendInfo)
  ...
  getMessages = async (threadID: string, pagination: PaginationArg): Promise<Paginated<Message>> => {
    let messages;
    try {
      messages = await this.api.getMessages(threadID, pagination, this.currentUser.id)
    } catch (e) {
  ...
  sendMessage = async (threadID: string, content: MessageContent, msgSendOptions: MessageSendOptions) => {
    await this.api.sendText(threadID, content.text, this.currentUser.id)
    return true
  }
  ...
  serializeSession = () => { return {sessionCookie: this.api.sessionCookie, nonceCookie: this.api.nonceCookie} }
