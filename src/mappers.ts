...
import type { SnapchatUser, SnapchatThread, SnapchatMessage } from './snapchat-types'
...
export function mapUser(user: SnapchatUser): User {
  if (!user) return
  return {
    id: user.id,
    username: user.mutable_username,
    fullName: user.display,
    imgURL: `https://sdk.bitmoji.com/render/panel/${user.bitmoji_selfie_id}-${user.bitmoji_avatar_id}-v1.webp?transparent=1&scale=1`
  }
}
...
export function mapThread(thread: SnapchatThread, currentUserSc: SnapchatUser): Thread {
  const scParticipants = thread.participants as SnapchatUser[]
  const participants = orderBy(
    scParticipants.map(p => mapUser(p)).filter(Boolean),
    u => u.id === currentUserSc.id,
  )
...
export const mapMessages = (messages: SnapchatMessage[], thread: SnapchatThread, currentUserID: string): Message[] =>
...
export function mapThreads(json: any, currentUser: SnapchatUser): Thread[] {
  ...
  const map = (t: any) => {
    const thread = mapThread(t.thread, currentUser)
    ...
    return {
      ...
      //     //   timestamp: new Date(),
      //     //   reactions: [],
      //     //   seen: { '895667967182163968': new Date() },
      //     //   isSender: true,
      //     //   senderID: '12345',
      //     //   text: 'Test message'
      //     // }
  ...
  //   {
  //   _original: '[{"message":{"id":"1606008817073983504","time":"1671737314934","affects_sort":true,"request_id":"28AAD397-6117-444D-9708-7D804A2BEE2C","conversation_id":"3942047054-895667967182163968","message_data":{"id":"1606008817073983504","time":"1671737314000","recipient_id":"3942047054","sender_id":"895667967182163968","text":"Hey! Following up on Farza’s tweet for uni students"}}}]',
  //   id: '1606008817073983504',
  //   timestamp: 2022-12-22T19:28:34.934Z,
  //   reactions: [],
  //   seen: { '895667967182163968': 1970-01-01T00:00:00.001Z },
  //   isSender: true,
  //   senderID: '895667967182163968',
  //   text: 'Hey! Following up on Farza’s tweet for uni students'
  // },

  // @ts-ignore
  json.forEach((t: any) => threads.push(map(t)))
  return threads
